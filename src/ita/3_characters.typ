#import "../../template.typ": keep_together, matrix_d6, rpg_table

// TODO: tradurre in italiano

#pagebreak()
== Personaggi

In Bloodstained Fangs, siete *vampiri, licantropi, streghe*—creature aggrappate agli ultimi brandelli della vostra umanità mentre vivete nell'ombra. Potreste essere un *vampiro neonato* che sta ancora imparando a cacciare. Un *cacciatore di vampiri itinerante* con le mani sporche di sangue. Un *lupo mannaro maledetto* che combatte la bestia interiore. Forse siete un membro appena iniziato di un'organizzazione di Cacciatori di Vampiri, gettato in una guerra che capite a malapena. O un *agente segreto* appena trasferito alla Fringe Science Division, dove i casi non finiscono nei rapporti ufficiali.

Ogni personaggio porta *il peso della propria condizione—o della propria conoscenza*. Poteri potenti. Nemici mortali. Alcuni troveranno potere, alleati, una pace fragile. Altri cadranno nell'oscurità, perderanno la loro umanità, diventeranno i mostri che temevano. Anche coloro che cacciano le creature della notte rischiano di diventare ciò che cacciano. *Nessuno è al sicuro dalla corruzione*.

Il vostro viaggio sarà fatto di *mistero, orrore e sangue*, dove ogni passo vi trascina *più a fondo nel mondo sotterraneo*. Riuscirete a farvi strada verso la salvezza e la libertà? O la vostra storia finirà in urla e silenzio?

=== Tutto è un Personaggio!

In _Bloodstained Fangs_ i Personaggi Non Giocanti (PNG), i Nemici, le Organizzazioni, i Mostri e persino gli oggetti rilevanti come i veicoli sono anch'essi personaggi!

Il tuo Protagonista è descritto da alcuni tratti fissi:

- *Nome*: Il nome dovrebbe essere iconico e coerente con il tono e l'ambientazione della storia.
- *Concetto*: Una descrizione concisa della professione, del background e delle abilità del personaggio. I migliori sono accoppiamenti aggettivo-nome, come _"Contrabbandiere Avventuroso"_ o _"Bambino Prodigio"_.
- *Abilità* (x2): Capacità non necessariamente specifiche del personaggio ma non caratteristiche comuni a tutti. _"Intelligente"_ non è un'abilità, _"Sussurratore di Motori"_ lo è.
- *Fragilità*: Qualcosa che potrebbe potenzialmente ostacolare il personaggio, fisicamente, mentalmente o socialmente.
- *Equipaggiamento* (x2): Equipaggiamento particolare fornito al personaggio in coerenza con l'ambientazione. Gli oggetti di uso quotidiano sono dati per scontati e non rientrano in questo tratto.
- *Obiettivo*: L'obiettivo a lungo termine.
- *Motivazione*: Ciò che spinge al perseguimento dell'obiettivo.
- *Nemesi*: Una persona o un'organizzazione che ostacola il protagonista. Può emergere durante le prime sessioni di gioco, può essere o meno l'antagonista diretto della storia, pronto ad apparire per rendere la vita ancora più difficile.
- *Fortuna*: La misura della capacità di un personaggio di evitare la sfortuna o un esito infausto. Si applica solo nei Conflitti e si ricarica automaticamente quando terminano. La Fortuna inizia e ha un massimo di 6.

Questi *tratti* sono descritti da *tag*, parole o frasi descrittive che possono identificare qualsiasi cosa nel mondo di gioco. Anche i *dettagli* dell'ambiente in cui si muove l'azione e le *condizioni* (fisiche o mentali) dei personaggi sono tag.

*Sono rappresentazioni qualitative. Non sono misure quantitative.*

#keep_together[
  == Tratti del Personaggio

  Puoi usare questi tratti per creare un personaggio al volo, mescolarli o semplicemente usarli come ispirazione.

  #matrix_d6(
    (
      [Cacciatore Incallito],
      [Vampiro Neonato],
      [Strega della Luna],
      [Licantropo Spericolato],
      [Lupo Mannaro Alpha],
      [Motociclista Tosto],
      [Topo di Biblioteca Occulta],
      [Storico Preoccupato],
      [Detective Determinato],
      [Federale Sotto Copertura],
      [Mago Nomade],
      [Tossicodipendente Disperato],
      [Leader della Congrega],
      [Mago Antico],
      [Abitante della Notte],
      [Acchiappafantasmi Amatoriale],
      [Esorcista Vaticano],
      [Hobbista Eccentrico],
      [Acchiappafantasmi Impavido],
      [Agente Disilluso],
      [Turista Smarrito],
      [Vampiro Ribelle],
      [Armaiolo Arcano],
      [Cercatore di Reliquie],
      [Medium Spettrale],
      [Scienziato Esiliato],
      [Diplomatico delle Ombre],
      [Gangster Zuccone],
      [Gangster Astuto],
      [Contrabbandiere Furtivo],
      [Divinatore Talentuoso],
      [Fuggitivo Perseguitato],
      [Vampiro Anziano],
      [Giovane Lupo Mannaro],
      [Rinomato Assassino],
      [Potente Politico],
    ),
  )
]

#keep_together[
  === Abilità

  #matrix_d6(
    (
      [Antiche Conoscenze],
      [Combattimento di Strada],
      [Armi da Fuoco],
      [Magia Lunare],
      [Prescienza],
      [Hacking],
      [Elettronica],
      [Spionaggio],
      [Biologia],
      [Maledizioni],
      [Esorcismo],
      [Diplomazia],
      [Intimidazione],
      [Chimica],
      [Furtività],
      [Scassinare Casseforti],
      [Comunione Spettrale],
      [Scassinare Serrature],
      [Esplosivi],
      [Alchimia],
      [Magia del Sangue],
      [Burocrazia],
      [Legge],
      [Tracciare],
      [Arti Marziali],
      [Fascino del Sangue],
      [Telecinesi],
      [Archeologia],
      [Parkour],
      [Seduzione],
      [Primo Soccorso],
      [Scassinare Casseforti],
      [Tattiche di Guerriglia],
      [Negromanzia],
      [Parlare con gli Animali],
      [Arrampicarsi],
    ),
  )]

#keep_together[
  === Fragilità

  #matrix_d6(
    (
      [Fede Forte],
      [Paura della violenza],
      [Credulone],
      [Moglie/Marito],
      [Mentore Arrabbiato],
      [Ideologia Feroce],
      [Giustizia Sociale],
      [Autoritario],
      [Odia le Autorità],
      [Braccato],
      [Ghoul],
      [Non sa mentire],
      [Terrore del fuoco],
      [Trauma infantile],
      [Terrore di annegare],
      [Alcolizzato],
      [Pieno di debiti],
      [Ricco e famoso],
      [Paura del buio],
      [Paura di ferire i mortali],
      [Persona amata mortale],
      [Atteggiamento Pomposo],
      [Arto maledetto],
      [Maledizione di morte],
      [Anarchico],
      [Auto-sabotaggio],
      [Odio per se stessi],
      [Ossessionato],
      [Insonnia cronica],
      [Ricercato],
      [Non sa nuotare],
      [Problemi di rabbia],
      [Tossicodipendente],
      [Zoppo],
      [Forte accento],
      [Cagionevole],
    ),
  )
]

#keep_together[
  === Equipaggiamento

  #matrix_d6(
    (
      [Pistola Silenziata],
      [Pugnale d'Argento],
      [Simbolo di Fede],
      [Anello Telecinetico],
      [Penna Infestata],
      [Fiala di Sangue di Vampiro],
      [Rilevatore Magico],
      [Chiave Incantata],
      [Mantello dell'Invisibilità],
      [Fucile a Pompa],
      [Collana di Zanne],
      [Medaglione dell'Antenato],
      [Katana Antica],
      [Jammer Radio],
      [Grimaldelli],
      [Coltelli da Lancio],
      [Balestra],
      [Sacca di sangue],
      [Bacchetta d'Osso],
      [Pietra di Luna],
      [Tomo Arcano],
      [Tomo Maledetto],
      [Distintivo della Polizia],
      [Acqua Santa],
      [Torcia UV],
      [Rilevatore di Fantasmi],
      [Giubbotto Tattico],
      [Documento Falso],
      [Lampada Solare],
      [Motocicletta],
      [Mappa Schizzata],
      [Scopa Magica],
      [Scudo Magico],
      [Visore Notturno],
      [Cacciavite Magico],
      [Pistola da Borsetta],
    ),
  )
]


#keep_together[
  === Nomi

  ==== Nomi Femminili

  #matrix_d6(
    (
      [Amber],
      [Isabella],
      [Selene],
      [Willow],
      [Bianca],
      [Lilith],
      [Violet],
      [Celeste],
      [Delilah],
      [Ember],
      [Chloe],
      [Vanessa],
      [Luna],
      [Iside],
      [Ophelia],
      [Thea],
      [Theodora],
      [Hannah],
      [Victoria],
      [Zara],
      [Rosemary],
      [Scarlet],
      [Zoey],
      [Rebecca],
      [Judith],
      [Prudence],
      [Sarah],
      [Morgana],
      [Jasmine],
      [Aurora],
      [Isadora],
      [Morgana],
      [Tamara],
      [Jessica],
      [Juliet],
      [Meave],
    ),
  )
]

#keep_together[
  ==== Nomi Maschili

  #matrix_d6(
    (
      [Damien],
      [Elliot],
      [Mark],
      [Adrian],
      [Xavier],
      [Jacob],
      [Silas],
      [Zachary],
      [Cassian],
      [Nathan],
      [Eric],
      [Cole],
      [Diego],
      [Edward],
      [Dominique],
      [Bob],
      [Andrew],
      [Samuel],
      [Marcus],
      [Vincent],
      [Ezekiel],
      [Michael],
      [Kieran],
      [Lucian],
      [Asher],
      [Alex],
      [Felix],
      [Kael],
      [Vito],
      [Daniel],
      [Luca],
      [Paul],
      [Liam],
      [Dean],
      [Dorian],
      [Matt],
    ),
  )
]

#keep_together[
  ==== Cognomi

  #matrix_d6(
    (
      [Cross],
      [Caine],
      [Frost],
      [Blackwood],
      [Night],
      [Irons],
      [Christie],
      [Kennedy],
      [Sterling],
      [Lancaster],
      [Darkmoon],
      [Flint],
      [Mulder],
      [Skully],
      [Stark],
      [Moore],
      [Cullen],
      [Winchester],
      [Bloodworth],
      [Carmichael],
      [Ravenscroft],
      [Bishop],
      [Evans],
      [Crowley],
      [Mercer],
      [Wolf],
      [Ashworth],
      [Winters],
      [Graves],
      [Cross],
      [Black],
      [White],
      [Reed],
      [Alliwell],
      [Bell],
      [Smith],
    ),
  )]

#keep_together[
  === Soprannomi

  #matrix_d6(
    (
      [Ombra],
      [Trinity],
      [Inverno],
      [Crepuscolo],
      [Alba],
      [Tramonto],
      [Ago],
      [Spina],
      [Proiettile],
      [Belladonna],
      [Sanguisuga],
      [Cannone],
      [Ombra],
      [Spettro],
      [Malocchio],
      [Mietitore],
      [L'Appeso],
      [Increspatura],
      [Nyx],
      [Fenice],
      [Mezzanotte],
      [Imbrunire],
      [Lama],
      [Filo],
      [Tagliente],
      [Corvo],
      [Segugio],
      [Atomo],
      [Teschio],
      [Fantasma],
      [Stalker],
      [Randagio],
      [Silenzio],
      [Master Mirror],
      [Duca],
      [Cremisi],
    ),
  )]

